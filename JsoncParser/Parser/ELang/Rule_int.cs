namespace Global.Parser.ELang {
  ...
  sealed public class Rule_int:Rule
  {
    private Rule_int(String spelling, List<Rule> rules) :
    base(spelling, rules)
    {
    }
    ...
    public static Rule_int Parse(ParserContext context)
    {
      context.Push("int");
      ...
      if (parsed)
      {
          rule = new Rule_int(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
      }
      else
      {
          context.index = s0;
