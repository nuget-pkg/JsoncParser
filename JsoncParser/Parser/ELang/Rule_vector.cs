namespace Global.Parser.ELang {
  ...
  sealed public class Rule_vector:Rule
  {
    private Rule_vector(String spelling, List<Rule> rules) :
    base(spelling, rules)
    {
    }
    ...
    public static Rule_vector Parse(ParserContext context)
    {
      context.Push("vector");
      ...
      parsed = false;
      {
        ...
        if (parsed)
        {
          ...
          {
            rule = Rule_begin_vector.Parse(context);
            if ((f1 = rule != null))
            {
                ...
                while (f2)
                {
                  ...
                  parsed = false;
                  {
                    ...
                    if (parsed)
                    {
                      ...
                      {
                        rule = Rule_value_separator.Parse(context);
                        if ((f3 = rule != null))
                        {
        ...
        if (parsed)
        {
          ...
          for (int i1 = 0; i1 < 1 && f1; i1++)
          {
            rule = Rule_end_vector.Parse(context);
            if ((f1 = rule != null))
            {
              a1.Add(rule, context.index);
              c1++;
      ...
      if (parsed)
      {
          rule = new Rule_vector(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
      }
      else
      {
          context.index = s0;
      ...
      context.Pop("vector", parsed);

      return (Rule_vector)rule;
    }
  }
}
