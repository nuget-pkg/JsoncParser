namespace Global.Parser.ELang {
  ...
  sealed public class Rule_object:Rule
  {
    private Rule_object(String spelling, List<Rule> rules) :
    base(spelling, rules)
    {
    }
    ...
    public static Rule_object Parse(ParserContext context)
    {
      context.Push("object");
      ...
      parsed = false;
      {
        ...
        if (parsed)
        {
          ...
          {
            rule = Rule_begin_object.Parse(context);
            if ((f1 = rule != null))
            {
        ...
        if (parsed)
        {
          ...
          for (int i1 = 0; i1 < 1 && f1; i1++)
          {
            rule = Rule_end_object.Parse(context);
            if ((f1 = rule != null))
            {
              a1.Add(rule, context.index);
              c1++;
      ...
      if (parsed)
      {
          rule = new Rule_object(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
      }
      else
      {
          context.index = s0;
      ...
      context.Pop("object", parsed);

      return (Rule_object)rule;
    }
  }
}
