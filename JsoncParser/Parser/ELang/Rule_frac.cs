namespace Global.Parser.ELang {
  ...
  sealed public class Rule_frac:Rule
  {
    private Rule_frac(String spelling, List<Rule> rules) :
    base(spelling, rules)
    {
    }
    ...
    public static Rule_frac Parse(ParserContext context)
    {
      ...
      if (parsed)
      {
          rule = new Rule_frac(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
      }
      else
      {
          context.index = s0;
