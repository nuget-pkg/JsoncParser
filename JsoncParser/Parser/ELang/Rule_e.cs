namespace Global.Parser.ELang {
  ...
  sealed public class Rule_e:Rule
  {
    private Rule_e(String spelling, List<Rule> rules) :
    base(spelling, rules)
    {
    }
    ...
    public static Rule_e Parse(ParserContext context)
    {
      ...
      }
      {
        ...
        if (parsed)
        {
          ...
          for (int i1 = 0; i1 < 1 && f1; i1++)
          {
            rule = Terminal_NumericValue.Parse(context, "%x45", "[\\x45]", 1);
            if ((f1 = rule != null))
            {
              a1.Add(rule, context.index);
              c1++;
      ...
      if (parsed)
      {
          rule = new Rule_e(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
      }
      else
      {
          context.index = s0;
