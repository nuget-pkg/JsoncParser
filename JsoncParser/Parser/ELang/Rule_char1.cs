namespace Global.Parser.ELang {
  ...
  sealed public class Rule_char1:Rule
  {
    private Rule_char1(String spelling, List<Rule> rules) :
    base(spelling, rules)
    {
    }
    ...
    public static Rule_char1 Parse(ParserContext context)
    {
      context.Push("char1");
      ...
      parsed = false;
      {
        ...
        if (parsed)
        {
          ...
          {
            rule = Rule_unescaped1.Parse(context);
            if ((f1 = rule != null))
            {
      ...
      if (parsed)
      {
          rule = new Rule_char1(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
      }
      else
      {
          context.index = s0;
      ...
      context.Pop("char1", parsed);

      return (Rule_char1)rule;
    }
  }
}
