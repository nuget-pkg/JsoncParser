namespace Global.Parser.ELang {
  ...
  sealed public class Rule_exp:Rule
  {
    private Rule_exp(String spelling, List<Rule> rules) :
    base(spelling, rules)
    {
    }
    ...
    public static Rule_exp Parse(ParserContext context)
    {
      ...
      if (parsed)
      {
          rule = new Rule_exp(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
      }
      else
      {
          context.index = s0;
