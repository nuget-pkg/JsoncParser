namespace Global.Parser.ELang {
  ...
  sealed public class Rule_as_is:Rule
  {
    private Rule_as_is(String spelling, List<Rule> rules) :
    base(spelling, rules)
    {
    }
    ...
    public static Rule_as_is Parse(ParserContext context)
    {
      context.Push("as-is");
                ...
                for (int i2 = 0; i2 < 1 && f2; i2++)
                {
                  ...
                  parsed = false;
                  {
                    ...
                    if (parsed)
                    {
                      ...
                      {
                        rule = Terminal_NumericValue.Parse(context, "%x00-3F", "[\\x00-\\x3F]", 1);
                        if ((f3 = rule != null))
                        {
                  ...
                  }
                  {
                    ...
                    if (parsed)
                    {
                      ...
                      for (int i3 = 0; i3 < 1 && f3; i3++)
                      {
                        rule = Terminal_NumericValue.Parse(context, "%x41-FFFFFFFF", "[\\x41-\\uFFFFFFFF]", 1);
                        if ((f3 = rule != null))
                        {
                          a3.Add(rule, context.index);
                          c3++;
      ...
      if (parsed)
      {
          rule = new Rule_as_is(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
      }
      else
      {
          context.index = s0;
      ...
      context.Pop("as-is", parsed);

      return (Rule_as_is)rule;
    }
  }
}
