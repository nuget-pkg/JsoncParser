namespace Global.Parser.ELang {
  ...
  sealed public class Rule_array:Rule
  {
    private Rule_array(String spelling, List<Rule> rules) :
    base(spelling, rules)
    {
    }
    ...
    public static Rule_array Parse(ParserContext context)
    {
      context.Push("array");
      ...
      parsed = false;
      {
        ...
        if (parsed)
        {
          ...
          {
            rule = Rule_begin_array.Parse(context);
            if ((f1 = rule != null))
            {
        ...
        if (parsed)
        {
          ...
          for (int i1 = 0; i1 < 1 && f1; i1++)
          {
            rule = Rule_end_array.Parse(context);
            if ((f1 = rule != null))
            {
              a1.Add(rule, context.index);
              c1++;
      ...
      if (parsed)
      {
          rule = new Rule_array(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
      }
      else
      {
          context.index = s0;
      ...
      context.Pop("array", parsed);

      return (Rule_array)rule;
    }
  }
}
