namespace Global.Parser.ELang {
  ...
  sealed public class Rule_decimal_point:Rule
  {
    private Rule_decimal_point(String spelling, List<Rule> rules) :
    base(spelling, rules)
    {
    }
    ...
    public static Rule_decimal_point Parse(ParserContext context)
    {
      ...
      if (parsed)
      {
          rule = new Rule_decimal_point(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
      }
      else
      {
          context.index = s0;
      ...
      return (Rule_decimal_point)rule;
    }
  }
}
