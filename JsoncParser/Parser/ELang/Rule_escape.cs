namespace Global.Parser.ELang {
  ...
  sealed public class Rule_escape:Rule
  {
    private Rule_escape(String spelling, List<Rule> rules) :
    base(spelling, rules)
    {
    }
    ...
    public static Rule_escape Parse(ParserContext context)
    {
      ...
      if (parsed)
      {
          rule = new Rule_escape(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
      }
      else
      {
          context.index = s0;
