namespace Global.Parser.ELang {
  ...
  sealed public class Rule_comment:Rule
  {
    private Rule_comment(String spelling, List<Rule> rules) :
    base(spelling, rules)
    {
    }
    ...
    public static Rule_comment Parse(ParserContext context)
    {
      ...
      parsed = false;
      {
        ...
        if (parsed)
        {
          ...
          for (int i1 = 0; i1 < 1 && f1; i1++)
          {
            rule = Rule_inmlcomment.Parse(context);
            if ((f1 = rule != null))
            {
              a1.Add(rule, context.index);
              c1++;
      ...
      if (parsed)
      {
          rule = new Rule_comment(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
      }
      else
      {
          context.index = s0;
