namespace Global.Parser.StrictJson {
  ...
  sealed public class Rule_quotation_mark:Rule
  {
    private Rule_quotation_mark(String spelling, List<Rule> rules) :
    base(spelling, rules)
    {
    }
    ...
    public static Rule_quotation_mark Parse(ParserContext context)
    {
      ...
      if (parsed)
      {
          rule = new Rule_quotation_mark(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
      }
      else
      {
          context.index = s0;
