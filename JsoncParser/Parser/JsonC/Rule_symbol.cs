namespace Global.Parser.JsonC {
  ...
  sealed public class Rule_symbol:Rule
  {
    private Rule_symbol(String spelling, List<Rule> rules) :
    base(spelling, rules)
    {
    }
    ...
    public static Rule_symbol Parse(ParserContext context)
    {
      context.Push("symbol");
          ...
          for (int i1 = 0; i1 < 1 && f1; i1++)
          {
            ...
            }
            {
              ...
              if (parsed)
              {
                ...
                for (int i2 = 0; i2 < 1 && f2; i2++)
                {
                  rule = Terminal_NumericValue.Parse(context, "%x41-51", "[\\x41-\\x51]", 1);
                  if ((f2 = rule != null))
                  {
                    a2.Add(rule, context.index);
                    c2++;
      ...
      if (parsed)
      {
          rule = new Rule_symbol(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
      }
      else
      {
          context.index = s0;
      ...
      context.Pop("symbol", parsed);

      return (Rule_symbol)rule;
    }
  }
}
